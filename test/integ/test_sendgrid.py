...
class UnitTests(unittest.TestCase):
    ...
    def test_api_keys_post(self):
        data = {"name": "My API Key", "sample": "data", "scopes": ["mail.send", "alerts.create", "alerts.read"]}
        headers = {"X-Mock": 201}
    ...
    def test_api_keys__api_key_id__put(self):
        data = {"name": "A New Hope", "scopes": ["user.profile.read", "user.profile.update"]}
    ...
    def test_asm_groups_post(self):
        data = {"description": "Suggestions for products our users might like.", "is_default": True, "name": "Product Suggestions"}
        headers = {"X-Mock": 201}
    ...
    def test_asm_groups__group_id__patch(self):
        data = {"description": "Suggestions for items our users might like.", "id": 103, "name": "Item Suggestions"}
    ...
    def test_asm_groups__group_id__suppressions_search_post(self):
        data = {"recipient_emails": ["exists1@example.com", "exists2@example.com", "doesnotexists@example.com"]}
        group_id = "test_url_param"
        headers = {"X-Mock": 200}
    ...
    def test_campaigns__campaign_id__patch(self):
        data = {"categories": ["summer line"], "html_content": "<html><head><title></title></head><body><p>Check out our summer line!</p></body></html>", "plain_content": "Check out our summer line!", "subject": "New Products for Summer!", "title": "May Newsletter"}
    ...
    def test_contactdb_recipients_patch(self):
        data = [{"email": "jones@example.com", "first_name": "Guy", "last_name": "Jones"}]
        headers = {"X-Mock": 201}
    ...
    def test_contactdb_recipients_post(self):
        data = [{"age": 25, "email": "example@example.com", "first_name": "", "last_name": "User"}, {"age": 25, "email": "example2@example.com", "first_name": "Example", "last_name": "User"}]
        headers = {"X-Mock": 201}
    ...
    def test_contactdb_segments_post(self):
        data = {
            "conditions": [{"and_or": "", "field": "last_name", "operator": "eq", "value": "Miller"}, {"and_or": "and", "field": "last_clicked", "operator": "gt", "value": "01/02/2015"}, {"and_or": "or", "field": "clicks.campaign_identifier", "operator": "eq", "value": "513"}],
            "list_id": 4,
    ...
    def test_contactdb_segments__segment_id__patch(self):
        data = {"conditions": [{"and_or": "", "field": "last_name", "operator": "eq", "value": "Miller"}], "list_id": 5, "name": "The Millers"}
    ...
    def test_mail_send_post(self):
        data = {
            ...
            "attachments": [{"content": "[BASE64 encoded content block here]", "content_id": "ii_139db99fdb5c3704", "disposition": "inline", "filename": "file1.jpg", "name": "file1", "type": "jpg"}],
            "batch_id": "[YOUR BATCH ID GOES HERE]",
            "categories": ["category1", "category2"],
            "content": [{"type": "text/html", "value": "<html><p>Hello, world!</p><img src=[CID GOES HERE]></img></html>"}],
            "custom_args": {"New Argument 1": "New Value 1", "activationAttempt": "1", "customerAccountNumber": "[CUSTOMER ACCOUNT NUMBER GOES HERE]"},
            ...
            "mail_settings": {
                ...
                "footer": {"enable": True, "html": "<p>Thanks</br>The SendGrid Team</p>", "text": "Thanks,/n The SendGrid Team"},
                "sandbox_mode": {"enable": False},
                "spam_check": {"enable": True, "post_to_url": "http://example.com/compliance", "threshold": 3},
            },
            "personalizations": [
                {
            ...
            "sections": {"section": {":sectionName1": "section 1 text", ":sectionName2": "section 2 text"}},
            "send_at": 1409348513,
            "subject": "Hello, World!",
            "template_id": "[YOUR TEMPLATE ID GOES HERE]",
            "tracking_settings": {
                "click_tracking": {"enable": True, "enable_text": True},
                "ganalytics": {"enable": True, "utm_campaign": "[NAME OF YOUR REFERRER SOURCE]", "utm_content": "[USE THIS SPACE TO DIFFERENTIATE YOUR EMAIL FROM ADS]", "utm_medium": "[NAME OF YOUR MARKETING MEDIUM e.g. email]", "utm_name": "[NAME OF YOUR CAMPAIGN]", "utm_term": "[IDENTIFY PAID KEYWORDS HERE]"},
                "open_tracking": {"enable": True, "substitution_tag": "%opentrack"},
                "subscription_tracking": {"enable": True, "html": "If you would like to unsubscribe and stop receiving these emails <% clickhere %>.", "substitution_tag": "<%click here%>", "text": "If you would like to unsubscribe and stop receiving these emails <% click here %>."},
            },
        }
        headers = {"X-Mock": 202}
    ...
    def test_subusers_post(self):
        data = {"email": "John@example.com", "ips": ["1.1.1.1", "2.2.2.2"], "password": "johns_password", "username": "John@example.com"}
        headers = {"X-Mock": 200}
    ...
    def test_suppression_spam_report__email__get(self):
        email = "test_url_param"
        headers = {"X-Mock": 200}
        response = self.sg.client.suppression.spam_reports._(email).get(request_headers=headers)
        self.assertEqual(response.status_code, 200)

    def test_suppression_spam_report__email__delete(self):
        email = "test_url_param"
        headers = {"X-Mock": 204}
        response = self.sg.client.suppression.spam_reports._(email).delete(request_headers=headers)
        self.assertEqual(response.status_code, 204)
    ...
    def test_templates__template_id__versions_post(self):
        data = {"active": 1, "html_content": "<%body%>", "name": "example_version_name", "plain_content": "<%body%>", "subject": "<%subject%>", "template_id": "ddb96bbc-9b92-425e-8979-99464621b543"}
        template_id = "test_url_param"
        headers = {"X-Mock": 201}
    ...
    def test_templates__template_id__versions__version_id__patch(self):
        data = {"active": 1, "html_content": "<%body%>", "name": "updated_example_name", "plain_content": "<%body%>", "subject": "<%subject%>"}
        template_id = "test_url_param"
    ...
    def test_user_webhooks_event_settings_patch(self):
        data = {"bounce": True, "click": True, "deferred": True, "delivered": True, "dropped": True, "enabled": True, "group_resubscribe": True, "group_unsubscribe": True, "open": True, "processed": True, "spam_report": True, "unsubscribe": True, "url": "url"}
        headers = {"X-Mock": 200}
    ...
    def test_user_webhooks_parse_settings_post(self):
        data = {"hostname": "myhostname.com", "send_raw": False, "spam_check": True, "url": "http://email.myhosthame.com"}
        headers = {"X-Mock": 201}
    ...
    def test_user_webhooks_parse_settings__hostname__patch(self):
        data = {"send_raw": True, "spam_check": False, "url": "http://newdomain.com/parse"}
    ...
    def test_whitelabel_domains__id__ips_post(self):
        ...
        id_ = "test_url_param"
        headers = {"X-Mock": 200}
        response = self.sg.client.whitelabel.domains._(id_).ips.post(request_body=data, request_headers=headers)
        self.assertEqual(response.status_code, 200)
    ...
    def test_whitelabel_domains__id__ips__ip__delete(self):
        ...
        ip = "test_url_param"
        headers = {"X-Mock": 200}
        response = self.sg.client.whitelabel.domains._(id_).ips._(ip).delete(request_headers=headers)
        self.assertEqual(response.status_code, 200)

    def test_whitelabel_domains__id__validate_post(self):
        id_ = "test_url_param"
        headers = {"X-Mock": 200}
        response = self.sg.client.whitelabel.domains._(id_).validate.post(request_headers=headers)
        self.assertEqual(response.status_code, 200)
    ...
    def test_whitelabel_ips__id__validate_post(self):
        id_ = "test_url_param"
        headers = {"X-Mock": 200}
        response = self.sg.client.whitelabel.ips._(id_).validate.post(request_headers=headers)
        self.assertEqual(response.status_code, 200)
    ...
    def test_whitelabel_links__id__patch(self):
        ...
        id_ = "test_url_param"
        headers = {"X-Mock": 200}
        response = self.sg.client.whitelabel.links._(id_).patch(request_body=data, request_headers=headers)
        self.assertEqual(response.status_code, 200)
    ...
    def test_whitelabel_links__id__delete(self):
        id_ = "test_url_param"
        headers = {"X-Mock": 204}
        response = self.sg.client.whitelabel.links._(id_).delete(request_headers=headers)
        self.assertEqual(response.status_code, 204)

    def test_whitelabel_links__id__validate_post(self):
        id_ = "test_url_param"
        headers = {"X-Mock": 200}
        response = self.sg.client.whitelabel.links._(id_).validate.post(request_headers=headers)
        self.assertEqual(response.status_code, 200)
