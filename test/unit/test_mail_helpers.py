...
response_content_with_all_three_mime_contents = json.dumps(
    {
        "content": [{"type": "text/plain", "value": "and easy to do anywhere, even with Python"}, {"type": "text/x-amp-html", "value": amp_html_content}, {"type": "text/html", "value": "<strong>and easy to do anywhere, even with Python</strong>"}],
        "from": {"email": "test+from@example.com", "name": "Example From Name"},
        "personalizations": [{"to": [{"email": "test+to@example.com", "name": "Example To Name"}]}],
...
class UnitTests(unittest.TestCase):
    ...
    def test_send_a_single_email_to_multiple_recipients(self):
        ...
        message = Mail(
            from_email=From("test+from@example.com", "Example From Name"), to_emails=to_emails, subject=Subject("Sending with SendGrid is Fun"), plain_text_content=PlainTextContent("and easy to do anywhere, even with Python"), html_content=HtmlContent("<strong>and easy to do anywhere, even with Python</strong>")
        )
    ...
    def test_single_email_with_amp_and_html_contents_to_single_recipient(self):
        ...
        self.maxDiff = None
        message = Mail(
            from_email=From("test+from@example.com", "Example From Name"), to_emails=To("test+to@example.com", "Example To Name"), subject=Subject("Sending with SendGrid is Fun"), amp_html_content=AmpHtmlContent(amp_html_content), html_content=HtmlContent("<strong>and easy to do anywhere, even with Python</strong>")
        )

        response_content = json.dumps(
            {
                "content": [{"type": "text/x-amp-html", "value": amp_html_content}, {"type": "text/html", "value": "<strong>and easy to do anywhere, even with Python</strong>"}],
                "from": {"email": "test+from@example.com", "name": "Example From Name"},
                "personalizations": [{"to": [{"email": "test+to@example.com", "name": "Example To Name"}]}],
                "subject": "Sending with SendGrid is Fun",
            }
    ...
    def test_single_email_with_amp_and_plain_contents_to_single_recipient(self):
        ...
        self.maxDiff = None
        message = Mail(
            from_email=From("test+from@example.com", "Example From Name"), to_emails=To("test+to@example.com", "Example To Name"), subject=Subject("Sending with SendGrid is Fun"), plain_text_content=PlainTextContent("and easy to do anywhere, even with Python"), amp_html_content=AmpHtmlContent(amp_html_content)
        )

        response_content = json.dumps(
            {
                "content": [{"type": "text/plain", "value": "and easy to do anywhere, even with Python"}, {"type": "text/x-amp-html", "value": amp_html_content}],
                "from": {"email": "test+from@example.com", "name": "Example From Name"},
                "personalizations": [{"to": [{"email": "test+to@example.com", "name": "Example To Name"}]}],
                "subject": "Sending with SendGrid is Fun",
            }
    ...
    def test_dynamic_template_data(self):
        ...
        to_emails = [To(email="test+to+0@example.com", name="Example To 0 Name", dynamic_template_data=DynamicTemplateData({"name": "Example 0 Name"})), To(email="test+to+1@example.com", name="Example To 1 Name", dynamic_template_data={"name": "Example 1 Name"})]
        message = Mail(from_email=From("test@example.com", "Example From Name"), to_emails=to_emails, subject=Subject("Hi!"), plain_text_content="Hello!", html_content="<strong>Hello!</strong>", is_multiple=True)
    ...
    def test_kitchen_sink(self):
        ...
        message.to = [To("test2@example.com", "Example User2", p=0), To("test3@example.com", "Example User3", p=0)]

        message.cc = Cc("test4@example.com", "Example User4", p=0)
        message.cc = [Cc("test5@example.com", "Example User5", p=0), Cc("test6@example.com", "Example User6", p=0)]

        message.bcc = Bcc("test7@example.com", "Example User7", p=0)
        message.bcc = [Bcc("test8@example.com", "Example User8", p=0), Bcc("test9@example.com", "Example User9", p=0)]
        ...
        message.substitution = [Substitution("%name2%", "Example Name 2", p=0), Substitution("%city2%", "Example City 2", p=0)]

        message.custom_arg = CustomArg("marketing1", "true", p=0)
        message.custom_arg = CustomArg("transactional1", "false", p=0)
        message.custom_arg = [CustomArg("marketing2", "false", p=0), CustomArg("transactional2", "true", p=0)]

        message.send_at = SendAt(1461775051, p=0)

        message.to = To("test10@example.com", "Example User10", p=1)
        message.to = [To("test11@example.com", "Example User11", p=1), To("test12@example.com", "Example User12", p=1)]

        message.cc = Cc("test13@example.com", "Example User13", p=1)
        message.cc = [Cc("test14@example.com", "Example User14", p=1), Cc("test15@example.com", "Example User15", p=1)]

        message.bcc = Bcc("test16@example.com", "Example User16", p=1)
        message.bcc = [Bcc("test17@example.com", "Example User17", p=1), Bcc("test18@example.com", "Example User18", p=1)]
        ...
        message.substitution = [Substitution("%name4%", "Example Name 4", p=1), Substitution("%city4%", "Example City 4", p=1)]

        message.custom_arg = CustomArg("marketing3", "true", p=1)
        message.custom_arg = CustomArg("transactional3", "false", p=1)
        message.custom_arg = [CustomArg("marketing4", "false", p=1), CustomArg("transactional4", "true", p=1)]
        ...
        message.content = [Content("text/calendar", "Party Time!!"), Content("text/custom", "Party Time 2!!")]

        message.attachment = Attachment(FileContent("base64 encoded content 1"), FileName("balance_001.pdf"), FileType("application/pdf"), Disposition("attachment"), ContentId("Content ID 1"))
        message.attachment = [
            Attachment(FileContent("base64 encoded content 2"), FileName("banner.png"), FileType("image/png"), Disposition("inline"), ContentId("Content ID 2")),
            Attachment(FileContent("base64 encoded content 3"), FileName("banner2.png"), FileType("image/png"), Disposition("inline"), ContentId("Content ID 3")),
        ]
        ...
        message.section = [Section("%section2%", "Substitution for Section 2 Tag"), Section("%section3%", "Substitution for Section 3 Tag")]
        ...
        message.custom_arg = [CustomArg("marketing6", "true"), CustomArg("transactional6", "false")]
        ...
        tracking_settings.subscription_tracking = SubscriptionTracking(True, SubscriptionText("Goodbye"), SubscriptionHtml("<strong>Goodbye!</strong>"), SubscriptionSubstitutionTag("unsubscribe"))
    ...
    def test_sendgrid_api_key(self):
        ...
        try:
            ...
            mail.add_content(Content("text/html", "<html><body>some SG.Ba2BlJSDba.232Ln2 here</body></html>"))
